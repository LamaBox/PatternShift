...
public partial class Main : Node2D
{
	...
	private AudioStreamPlayer _sound1;
	...
	public override void _Ready()
	{
		...
		_sound1 = GetNode<AudioStreamPlayer>("SoundsPlayer1");
		_sound2 = GetNode<AudioStreamPlayer>("SoundsPlayer2");
		if (IsFirstRun == true)
		{
			var PlayGuideScene = (PackedScene)GD.Load("res://scene/PlayGuide.tscn");
			var PlayGuideInstance = PlayGuideScene.Instantiate<Control>();
			AddChild(PlayGuideInstance);
			IsFirstRun = false; // перенести в конфиг
		}
	}
	...
	{
		//_music.Stream = ResourceLoader.Load<AudioStream>("res://music/gamemusic.mp3");
		//_music.Play();
		PlaySound1("res://sounds/клик 1.mp3");
	...
	//Расположние первых карт на поле
		...
		if (InfinityModeTrue == false)
		{
			if (CardObserver < 69)
			{
				if (count == 3)
				{
					...
					{
						//PlaySound2("res://sounds/верная комбинация 1.mp3");
						count = 0;
						int ic = 0;
						for (int c = 0; c < 12; c++)
						{
					...
					{
						GD.Print("Is not set");
						//PlaySound2("res://sounds/неверная комбинация 1.mp3");
						if (_timerover != null)
						{
						...
						{
							CardCount[list[c]] = 0;
							var CardDeck1 = ResourceLoader.Load<Texture2D>("res://CardTexture/card2.png");
			...
			else
			{
				...
				if (count == 3)
				{
					...
					if (ChekPattern(CardField[list[0]], CardField[list[1]], CardField[list[2]]))
					{
						...
						{
							GD.Print("Паттернов больше нет");
							//PlaySound2("res://sounds/победа.wav"); 
							if (_roundTimer != null) _roundTimer.Stop();
							//CardDesabled(); 
							for (int f = 0; f < 12; f++) GD.Print("Card - ", GetCardCode(CardField[f]));
						}
					}
					else
					{
	...
	{
		_time--;
		_timeLable.Text = $"{Mathf.Floor(_time / 60):00}:{Mathf.Floor(_time % 60):00}";
	}
	...
	private void TimerGameOverOut()
	{
		...
		CardDesabled();
		GD.Print("GameOver");
		PlaySound1("res://sounds/проигрыш.wav");
		//Вывод результатов
	...
	////Проигрыш звуков
	private void PlaySound1(string s)
	{
		_sound1.Stream = ResourceLoader.Load<AudioStream>(s);
		_sound1.Play();
	}

	private void PlaySound2(string s)
	{
		_sound2.Stream = ResourceLoader.Load<AudioStream>(s);
		_sound2.Play();
	}
	...
	//Звуки
	private void _on_button_1_mouse_entered()
	{
		PlaySound2("res://sounds/наведение на карточку 1.mp3");
	}
	...
	private void _on_button_4_mouse_entered()
	{
		PlaySound2("res://sounds/наведение на карточку 1.mp3");
	}
	private void _on_button_5_mouse_entered()
	{
		PlaySound2("res://sounds/наведение на карточку 1.mp3");
	}
	private void _on_button_6_mouse_entered()
	{
		PlaySound2("res://sounds/наведение на карточку 1.mp3");
	}
	private void _on_button_7_mouse_entered()
	{
		PlaySound2("res://sounds/наведение на карточку 1.mp3");
	}
	private void _on_button_8_mouse_entered()
	{
		PlaySound2("res://sounds/наведение на карточку 1.mp3");
	}
	private void _on_button_9_mouse_entered()
	{
		PlaySound2("res://sounds/наведение на карточку 1.mp3");
	}
	private void _on_button_10_mouse_entered()
	{
		PlaySound2("res://sounds/наведение на карточку 1.mp3");
	}
	private void _on_button_11_mouse_entered()
	{
		PlaySound2("res://sounds/наведение на карточку 1.mp3");
	}
	private void _on_button_12_mouse_entered()
	{
		PlaySound2("res://sounds/наведение на карточку 1.mp3");
	}
