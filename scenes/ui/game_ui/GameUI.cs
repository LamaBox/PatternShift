...
public partial class GameUI : Control
{
	...
	private static bool Lose = true;
	public static bool _isntTimeMode = true;
	...
	private static void CheckSet()
	{
		...
		switch (ModeSelect._selectedMode)
		{
			case "time_trial":
			...
			case "endless":
				...
				{
					GD.Print("It`s set!");
					GD.Print("SetCount = ", _setCount += 1);
					PlaySound1("res://sounds/верная комбинация 1.mp3");
					CardClickedCounter = 0;
					if (TheDeck.Count == 0)
					{
						UpdateTheDeck();
						GD.Print("UpdatedTheDeck");
	...
	{
		if (_isPaused || _isntTimeMode) return;
	...
	private static void AddAdditionalCards()
	{
		...
		{
			GD.Print("New 3 card doesn`t generate set. Update");
			for (int i = 12; i < 15; i++)
			{
				var shape = (Card.ShapeType)(TheDeck[0][0]);
				var color = (Card.ColorType)(TheDeck[0][1]);
	...
	{
		if (Lose) PlaySound2("res://sounds/проигрыш.wav");
	...
	//Тосовка неполной колоды для бесконечного мода
	private static void UpdateTheDeck()
	{
		int i = 12;
		if (IsAddCards) i = 15;
		int k = Discard.Count - i;
		for (int j = 0; j < k; j++) { TheDeck.Add(Discard[0]); Discard.RemoveAt(0); }
		Shuffle<int[]>(TheDeck);	
	}
}
